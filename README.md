# Swift-1AllForOne
**1. Pasos para crear un repositorio:**
```
  A. Primero se crea un token en GitHub
  B. Segundo en Xcode > Preferences > Accounts > (+) GitHub Account
  C. Tercero crear proyecto:
        Team:                     None
        Organization Identifier:  GabrielToroG
  D. Activar: "Source Control: Create Git repository on my mac"
  E. (Command + 2) > Repositories > (...) > New {ProductName} Remote... > Create
```

**2. Pasos para hacer push en un repositorio:**
```
  A. (Command + Option + C)
  B. Seleccionar cambios > Commit Message > Commit
  C. Barra superior > Source Control > Push > Seleccionar rama > Push
```

**3. Pasos para crear un proyecto:**
```
    A. Primero se crearon las carpetas:
        View
        Model
        ViewModel
        Assets      ->  imagenes, lotties, localazy
        Extensions  ->  Extensiones a elementos ya conocidos (String, UIImage, etc.)
        Helpers     ->  Funciones que NO van en View, Model, ViewModel y que se usan más de una vez
    B. Luego:
        se arrastra el archivo Assets a la carpeta Assets
        se arrastra el LaunchScreen a la carpeta View
    C. Finalmente:
        Se crea una carpeta llamada main en la carpeta View y aqui se arrastra el Storyboard y el ViewController inicial
```
